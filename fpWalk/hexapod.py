...
class Hexapod_12DOF(Hexapod):
    ...
    def step(self, steps=1, time_step=.02):
        for x in range(steps):
            self.right_left_right_step(time_step)
            self.left_right_left_step(time_step)
    ...
    def turn_left(self, steps=1, time_step=0.2):
        for x in range(steps):
    ...
    def turn_right(self, steps=1, time_step=0.2):
        for x in range(steps):
    ...
    def row(self, time_step=0.3, back=False):
    ...
    def rotate(self, steps=1, time_step=0.3, left=False, back=False):
        for x in range(steps):
            self.move_all_legs(rotate_value=100, raise_value=50)
            time.sleep(time_step)
